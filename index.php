...
<body>
    ...
    <script>
        ...
        async function showEditModal(id){
            ...
            let response = await fetch("http://localhost/Domaci3/api/user.php?id="+id);
            ...
            if(responseJSON.status == true){
                ...
                document.getElementById("editModalLastName").value = userData.last_name;
                document.getElementById("editModalEmail").value = userData.email;
                document.getElementById("editModalCountry").value= userData.country;  //ovo jos ne radi, treba selected, add atribute
                document.getElementById("editModalCity").value= userData.city;
                document.getElementById("country"+userData.country).setAttribute('selected', true);
                //document.getElementById("city"+userData.city).setAttribute('selected', true);
        ...
        async function getCities(id, IDjs){

            //sad ga barem cita sa back-a, ali ne radi jos :(
            let response1 = await fetch(`http://localhost/Domaci3/api/city.php?id=${id}`);
            let responseJson = await response1.json();
            if(responseJson.status == true){
                let cityData = responseJson.data;
                let cityHTML="";
                for(let i=0;i<cityData.length;i++){
                    cityHTML+=`<option id="city${cityData[i].name}" value="${cityData[i].name}">${cityData[i].name}</option>`;
                    //console.log(cityData[i]);
                }
                document.getElementById(IDjs).innerHTML=cityHTML;
            }
        }
